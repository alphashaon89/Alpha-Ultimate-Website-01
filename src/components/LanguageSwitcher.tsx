          ...
          >
            {languages.map((lang) => (
              <button
                ...
                className={`w-full text-left px-4 py-2.5 text-sm flex items-center justify-between hover:bg-[#14f195]/08 transition-colors ${
                  i18n.language === lang.code ? 'text-[#14f195] font-bold' : 'text-gray-400'
                }`}
              >
                {lang.label}
                {i18n.language === lang.code && <div className="w-1.5 h-1.5 rounded-full bg-[#14f195] shadow-[0_0_6px_rgba(20,241,149,0.8)]" />}
              </button>
            ))}
          </motion.div>
        )}
