...
export default function Booking() {
  ...
  if (isSubmitted) {
    return (
      <div className="bg-[#070712] text-white pt-24 pb-20 flex items-center justify-center min-h-screen">
              ...
              <div>
                ...
                <select name="service" value={formData.service} onChange={handleChange} className={SELECT_BASE}>
                  {content?.services.map((s: Service) => (
                    <option key={s.id} value={s.id}>{s.title[language] || s.title['en']}</option>
                  ))}
                </select>
              </div>
            </div>
